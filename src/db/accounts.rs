...
/// Append a binding row, making it the active binding.
///
/// On the first-ever bind (the accounts table was empty), also backfills
/// `documents.source_account_id` on every unstamped row: those rows entered
/// the database while it implicitly belonged to this account. A rebind never
/// backfills; provenance of already-present rows is a fact about the past.
///
/// Returns the number of backfilled documents (always 0 on rebind).
pub fn bind_account(
    conn: &Connection,
    account_id: &str,
...
mod tests {
    ...
    #[test]
    fn first_bind_backfills_existing_documents() {
        let conn = db_with_documents();

        let backfilled =
            bind_account(&conn, "user_01AAA", Some("uuid-1"), Some("old@example.com")).unwrap();
    ...
    #[test]
    fn rebind_appends_without_backfilling() {
        let conn = db_with_documents();
        bind_account(&conn, "user_01AAA", Some("uuid-1"), Some("old@example.com")).unwrap();

        // A document that arrives unstamped between binds (in practice this
        // cannot happen through sync, but the invariant is: only the first
        // bind ever backfills).
        conn.execute(
