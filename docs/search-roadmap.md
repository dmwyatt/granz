...
- **Keyword search** (Phase 1, #39): FTS5 `MATCH` with implicit-AND semantics (each term quoted individually; user-supplied quotes force phrase matching), ranked by `bm25()` with recency as tiebreak. Title substring matches rank as a tier above content matches in this mode; the hybrid pipeline weights title matches with the Phase 5 title boost instead. This retrieval serves `grans grep` (the complete-lookup verb, #65) and the lexical half of hybrid search.
- **Semantic retrieval** (inside hybrid): nomic-embed-text-v1.5 (768d) via fastembed, brute-force cosine over in-memory vectors. Chunkers for transcripts (adaptive window), panels (section), notes (paragraph). The standalone `--semantic` mode was removed via #59 once hybrid reached parity on the paraphrase stratum; the retriever is still benchmarkable directly with `benchmark quality --mode semantic`.
...
- **Hybrid search** (Phase 2, #40, promoted to the bare `grans search` default via #52 on 2026-07-11): runs both retrievers, truncates each ranked list to a 100-document candidate pool, and fuses by reciprocal rank fusion (k=60) in `query/fusion.rs` + `query/hybrid.rs`. Output is the fused meeting list, reranked by default (see next bullet). The surface split into two verbs via #65: `grans search` is this ranked pipeline (always), and plain FTS lookup became `grans grep`; the `--keyword` and `--hybrid` flags were removed.
...
- **Shaped output** (#57 part 1): hybrid results render as per-meeting cards with match evidence. Evidence degrades in tiers (`query/evidence.rs`): lexical match sites located with Rust-side matchers (`matches_all_tokens`; FTS5 `snippet()`/`highlight()` were considered but the notes/panels FTS indexes are whole-blob, so they cannot yield section labels); the reranker's best chunk when no query term appears literally; a bare `title match` card otherwise. Shaping is presentation-only, runs on the displayed page (not the candidate pool), and never reorders (pinned by test). The numeric score left human output (the ordering blend made the score column non-monotonic, which read as broken); `--json` carries score, signals, and highlight offsets. Part 2 finished the unification: keyword search renders the same cards, `--context` became a card expansion on any mode (neighboring units around each shown match; the one-result-per-match `ContextWindow` mode and its rendering stack were deleted), and `--speaker` became a match-evidence filter (only that speaker's utterances count; meetings with nothing attributable drop out), fixing the silently-ignored bare `--speaker` (#60); it later moved to grep alone (see next bullet). `--limit` counts meetings everywhere.
- **Two verbs, two contracts** (#65): `grans search` (`commands/search.rs`) is ranked discovery over bounded candidate pools, so it never claims a corpus total; it prints `Top N match(es)` and cross-links grep with the uncapped FTS match count (`keyword_total` on `HybridRanking`, also in search JSON). `grans grep` (`commands/grep.rs`) is complete lexical lookup whose `Found N meeting(s)` count is a fact; shared shaping/paging lives in `commands/search_common.rs`. The shape options (`--matches`, `--context`) ride on both verbs. `--speaker` is grep-only (semantic chunks carry no per-utterance attribution, so search could not honor it before its caps), and `--meeting` pushes down into both retriever candidate lists before pooling.
...
6. **One escape hatch, two diagnostic paths.** `grans grep` remains alongside ranked search, so a missed regression is recoverable (exact terms, no ML dependencies); it took over from `--keyword` when the surface split into verbs (#65). Diagnosis moved off user-facing modes when `--semantic` was removed (#59): per-result, the `signals` field in shaped JSON says which retrievers surfaced a meeting; retriever-level, `benchmark quality --mode semantic` talks to the semantic retriever directly.
...
The ceiling here is FTS itself: with FTS scoring zero outside exact-term, fusion has only one useful signal for mixed/paraphrase queries. Larger hybrid gains wait on reranking (Phase 3) and embedding improvements (Phase 4). Hybrid search was promoted to the bare `grans search` default via #52 on 2026-07-11.
...
Chunker cap fixes (2026-08-02, #123): the scheme parameters are unchanged, but four chunker defects were fixed: split-path chunks exceeding the cap by the overlap budget (74 live chunks, tails truncated at embed time), pure-carryover duplicate chunks with inverted window metadata (149 live chunks), null or stale `start_timestamp` on 97% of transcript chunk metadata, and uncapped panel/notes chunks that also ignored the header budget. About 0.6% of chunks change content and re-embed; the rest get a metadata-only refresh. Ledger entries recorded before the fix remain comparable as long as the frozen snapshot's embeddings are not regenerated; re-running `grans embed` against a snapshot copy now produces slightly different chunks in ~70 documents, so record a fresh baseline if the snapshot is ever re-embedded.
...
- Result shaping (#57): per-meeting cards with match evidence, spun out of Phase 5; part 2 unified keyword search onto the cards and re-homed `--context`/`--speaker` (fixing #60)
- Mode consolidation (#59): the standalone `--semantic` mode removed; hybrid + `--keyword` are the two retrieval modes
