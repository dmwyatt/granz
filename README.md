...
Both verbs render the same cards. Each card shows why the meeting matched: the source of the best match (`AI notes` with its section heading, `your notes`, or `transcript` with time and speaker, named when Granola attributed the utterance and `You`/`Other` otherwise), a snippet with the query terms highlighted, and a `+N more matches` line when the meeting matched in more places. `--matches N` shows up to N snippets per meeting (default 1), and `--context N` renders N neighboring units around each shown match inside the card (the utterances around a transcript hit, the sections around an AI-notes hit, the paragraphs around a notes hit), with the matched unit shown whole. In search results, a meeting that matched semantically but contains none of the query's literal words shows its best-matching passage without highlights, and a meeting that matched only by its title says `title match`. The relevance score is not shown in the card view; `--json` carries it (`score`), along with which retrievers surfaced each meeting (`signals`), the full match list, and snippet highlight offsets. `--min-score` drops search results below a relevance threshold; it takes a value between 0 and 1 (rerank scores are probabilities) and conflicts with `--fast`, since only the rerank stage produces that score. When a threshold rejects every candidate, the empty result says so and names the best score it turned away, so it never reads like a query nothing matched. Both verbs support `--in`, `--meeting`, date filters, and `--limit` (which counts meetings everywhere).
...
After a wait, grans checks that the release it fetched was published by the
build it waited for. If GitHub hands back an older release, grans stops with an
error rather than installing a binary you did not wait for; run the command
again in a moment.
